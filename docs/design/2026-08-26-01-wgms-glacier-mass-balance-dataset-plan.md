...
## Addendum — calculation method replaced (2026-08-28)

Following the investigation in
[docs/notes/2026-08-27-wgms-reference-glacier-mass-balance-discrepancy.md](../notes/2026-08-27-wgms-reference-glacier-mass-balance-discrepancy.md),
`WgmsGlacierMassBalanceSourceFileTransformer`'s anomaly-from-own-mean/flat-average
approach (described throughout the plan above) was replaced. The anomaly
technique made the index harder to read as "what is mass balance actually
doing over time" and turned out to be the dominant reason the index diverged
numerically from WGMS's own published reference-glacier figures - a
comparison worth being able to make even though the two were never intended
to be identical series.

**New design:** `WgmsGlacierMassBalanceSourceFileTransformer` now takes two
constructor parameters instead of being parameterless:

- `WgmsGlacierFilter filter` - `Benchmark` (>10 years, ≤1yr recent gap) or
  `Reference` (>30 years, same gap rule) - which of WGMS's own glacier
  categories qualifies a glacier to contribute.
- `WgmsAveragingStage averagingStage` - `OneStage` (flat mean of every
  qualifying glacier's raw annual balance) or `TwoStage` (mean within each
  glacier's GTN-G region first, then mean of region means - WGMS's own
  approach, avoiding bias toward densely-instrumented regions like the Alps;
  requires reading `glacier.csv`'s `gtng_region` column from the same zip).

Both parameters are simple enum switches, not a redesign of the download/
transform pipeline - `TransformingDataSetDownloader` wiring, the zip
download, and the CSV output shape (`Year,Value`) are unchanged.

**Values are now raw annual balance, never an anomaly.** No per-glacier
baseline is subtracted. This is both simpler to reason about (a reader can
compare the index's magnitude directly to a real mass-balance number, e.g.
"-1 m w.e. this year") and the metric WGMS's own published figures use, which
makes an apples-to-apples comparison against `mb_ref.csv` possible - the
`Reference`+`TwoStage` combination reproduces WGMS's published index to
within ~0.13 m w.e. mean absolute difference across the record (see the
notes doc and `WgmsReferenceGlacierMassBalanceTests.cs`).

**Shipped preset uses `Benchmark` + `TwoStage`** (`ClimateExplorer.Data.Misc/Program.cs`'s
downloader registration) - the wider, geographically broader Benchmark
glacier list, but with WGMS's own region-aware averaging. A dedicated test
(`TransformAsync_BenchmarkVsReferenceFilter_BothTwoStage_OnRealData_ProduceSimilarValues`)
confirms this stays close to the stricter `Reference`+`TwoStage` figure
(~0.035 m w.e. mean absolute difference) - the glacier-list choice matters
much less than the raw-value/two-stage-averaging choice did.

**Regenerated data:** `ClimateExplorer.WebApi/Datasets/Glaciers/wgms-glacier-mass-balance-index.csv`
and its `ClimateExplorer.SourceData/Glaciers/` mirror were regenerated by
running the actual (new) transformer against a freshly downloaded copy of
the same 2026-02-10 WGMS release, `Benchmark`+`TwoStage` - same 78 years
(1946-2025), same 138 qualifying glaciers, different (raw, region-averaged)
values throughout.

**Files touched (beyond the original implementation):**
- `ClimateExplorer.Data.Downloading/Transformers/WgmsGlacierMassBalanceSourceFileTransformer.cs` -
  rewritten: `WgmsGlacierFilter`/`WgmsAveragingStage` enums, constructor
  parameters, raw (not anomaly) values, optional region-aware two-stage
  averaging reading `glacier.csv`.
- `ClimateExplorer.Data.Misc/Program.cs` - downloader registration now passes
  `WgmsGlacierFilter.Benchmark, WgmsAveragingStage.TwoStage`.
- `ClimateExplorer.Core/DataSetDefinitionsBuilder/DataSetDefinitionsBuilder.cs` -
  dataset `Description` updated to describe the region-then-global averaging.
- `ClimateExplorer.Web.Client/UiModel/SuggestedPresetLists.Global.cs` - preset
  `Description` updated (no longer "anomaly").
- `ClimateExplorer.WebApi/Datasets/Glaciers/wgms-glacier-mass-balance-index.csv`
  and the `ClimateExplorer.SourceData/Glaciers/` mirror - regenerated, real
  data.
- `ClimateExplorer.UnitTests/DataSetSourceFileTransformerTests.cs` - Wgms
  tests rewritten for the new constructor/raw-value/two-stage behaviour (11
  tests, up from 8: added a two-stage test, a Benchmark-vs-Reference filter
  test, and two-stage-specific `glacier.csv`-missing/region-missing tests).
- `ClimateExplorer.UnitTests/WgmsReferenceGlacierMassBalanceTests.cs` -
  simplified to run the actual transformer (previously it duplicated the
  averaging logic to prototype what WGMS's method would produce, since the
  production code didn't support it yet).
- `ClimateExplorer.UnitTests/GlacierFixtures/glacier_regions.csv` - header
  changed from `glacier_id,gtng_region` to `id,gtng_region`, matching the
  real WGMS `glacier.csv` column name the transformer now reads.
