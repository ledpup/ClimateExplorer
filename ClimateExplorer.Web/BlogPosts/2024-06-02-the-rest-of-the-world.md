...
A question we had was: could k-means deal with [geodetic coordinates](https://en.wikipedia.org/wiki/Geodetic_coordinates) (i.e., longitude and latitude). A [stack-overflow question](https://stackoverflow.com/questions/24762435/clustering-geo-location-coordinates-lat-long-pairs-using-kmeans-algorithm-with) queried the issue; i.e., since k-means only uses [Euclidean distance](https://en.wikipedia.org/wiki/Euclidean_distance), would it work for places on the Earth - where we use the [haversine formula](https://en.wikipedia.org/wiki/Haversine_formula) to resolve the distances between locations with coordinates in latitude and longitude? The answer on stack-overflow was: "no, k-means will not work, use [density-based spatial clustering of applications with noise (DBSCAN)](https://en.wikipedia.org/wiki/DBSCAN) with the Haversine formula to calculate distance."
