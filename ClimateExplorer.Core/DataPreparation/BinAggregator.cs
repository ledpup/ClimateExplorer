...
public static class BinAggregator
{
    ...
    private static double? WeightedMean(IEnumerable<ContainerAggregate> data)
    {
        var containersWithData = data.Where(x => x.Value.HasValue);

        if (!containersWithData.Any())
        {
            return null;
        }
    ...
    private static double? Median(IEnumerable<ContainerAggregate> data)
    {
    ...
    private static Func<IEnumerable<ContainerAggregate>, double?> GetAggregationFunction(ContainerAggregationFunctions function, SeriesTransformations seriesTransformation)
    {
        switch (function)
        {
    ...
    private static ContainerAggregate BuildContainerAggregateForCup(Cup cup, Func<IEnumerable<ContainerAggregate>, double?> aggregationFunction)
    {
        return
    ...
    private static Bin[] AggregateBinsByRepeatedlyApplyingAggregationFunctions(
        RawBinWithDataAdequacyFlag[] rawBins,
    ...
    private static Bin[] AggregateBinsByApplyingAggregationFunctionOnceForAllDataPoints(
        RawBinWithDataAdequacyFlag[] rawBins,
    ...
    private static ContainerAggregate AggregateContainerAggregates(
    ...
    private struct AggregationIntermediate
    {
